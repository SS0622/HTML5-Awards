...
async function startGame() {
    ...
    return new Promise(resolve => {
        ...
        vid = video.addEventListener('canplaythrough', function drawAndPlay() {
...
function displayScenario(callback) {
    ...
    if (flagLength > 0) {
        ...
        for (let i = 0; i < flagNames.length; i++) {
            ...
            // 現在の選択肢に来るのが２回目以上の場合
            console.log(currentScenario.choicesFlag[i].length);
            if (dictionary.get(flagNames[i]) == 'selected') flagNum++;
                ...
                for (let j = 1; j < currentScenario.choicesFlag[i].length; j++) {
                    console.log(dictionary.get(currentScenario.choicesFlag[i][j]));
                    if (dictionary.get(currentScenario.choicesFlag[i][j]) != 'selected') {
                        console.log("selectedではなかったためfalse", i);
                        break;
                    }
                    if (j == currentScenario.choicesFlag[i].length - 1) {
                        console.log("trueになった");
                        dictionary.set(flagNames[i], 'true');
                    }
                }
            }
        }
    ...
    function draw() {
        ...
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
        ctx.drawImage(character, 200, -190, 384, 600);
            ...
            if (str == 'false') {
                ...
                for (let j = 2; j <= quantity; j++) {
                    ...
                    if (j == quantity) {
                        ...
                        console.log(dictionary.get(currentScenario.name[i]));
                        currentScenarioIndex = currentScenario.displayFlag[i][1];
                        ...
                        if (isInside(clickX, clickY, x, x + width, yc, yc + height)) {
                            if (deleteFlag) {
                                ...
                                if (either == 'start') {
                                    ...
                                    canvas.removeEventListener('canplaythrough', vid);
                                } else if (either == 'load') {
                                    sessionStorage.clear();
                                    dictionary = new Map();
                                    console.log(JSON.parse(localStorage.getItem(slotnum)));
                                    ...
                                    dictionary.set(-1, -1);
                                    saveData.selected = Array.from(dictionary);
                                    saveData.background = sessionStorage.getItem('backgroundsave');
                                    saveData.bgm = sessionStorage.getItem('bgmsave');
                                    saveData.character = sessionStorage.getItem('charactersave');
                                    console.log(saveData);
                                    localStorage.setItem(slotnum, JSON.stringify(saveData));
                                }
                                ctx.clearRect(0,0,canvas.width,canvas.height);
...
// すべての画像を読み込む処理
function loadAllImages(backs, chars) {
    return Promise.all([
        loadImage(background, backs),
        loadImage(character, chars),
    ]);
}
// ゲームスタート
async function main() {
